import { useState } from "react";

function SocialAuthModal({ provider, onClose, onSuccess }) {
  const [loading, setLoading] = useState(false);
  const [selectedAccount, setSelectedAccount] = useState({
    name: "Sunny Kumar",
    email: "sunnykumar6207058974@gmail.com",
    avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80",
  });

  if (!provider) return null;

  const isGoogle = provider === "google";

  const handleAuthenticate = () => {
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      onSuccess({
        provider,
        email: selectedAccount.email,
        name: selectedAccount.name,
        avatar: selectedAccount.avatar,
      });
    }, 1200);
  };

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content social-auth-modal" onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}>
          ✕
        </button>

        <div className="social-auth-container">
          {/* Header Icon */}
          <div className="social-auth-header">
            {isGoogle ? (
              <div className="google-icon-box">
                <svg width="36" height="36" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                  />
                  <path
                    fill="#34A853"
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                  />
                  <path
                    fill="#FBBC05"
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
                  />
                  <path
                    fill="#EA4335"
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
                  />
                </svg>
              </div>
            ) : (
              <div className="apple-icon-box">
                <svg width="36" height="36" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 6.32c.66-.8 1.11-1.92.99-3.04-.96.04-2.12.64-2.8 1.44-.61.71-1.15 1.86-1.01 2.96 1.07.08 2.16-.56 2.82-1.36z" />
                </svg>
              </div>
            )}
            <h3>Sign in with {isGoogle ? "Google" : "Apple"}</h3>
            <p>Choose an account to continue to <strong>Cartify</strong></p>
          </div>

          {/* Account selector list */}
          <div className="social-account-card">
            <div className="account-item selected">
              <img src={selectedAccount.avatar} alt="Avatar" className="user-avatar" />
              <div className="account-info">
                <strong>{selectedAccount.name}</strong>
                <span>{selectedAccount.email}</span>
              </div>
              <span className="check-mark">✓</span>
            </div>

            <div className="account-item-add" onClick={() => setSelectedAccount({
              name: "Sunny Kumar",
              email: "sunnykumar6207058974@gmail.com",
              avatar: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80"
            })}>
              <span className="plus-icon">+</span>
              <span>Use another {isGoogle ? "Google" : "Apple"} Account</span>
            </div>
          </div>

          {/* Authentication Action */}
          <button
            className={`btn-primary social-submit-btn ${isGoogle ? "google-btn-theme" : "apple-btn-theme"}`}
            onClick={handleAuthenticate}
            disabled={loading}
          >
            {loading ? (
              <span className="btn-loading">
                <span className="spinner-sm"></span> Authenticating & sending email...
              </span>
            ) : (
              `Continue as ${selectedAccount.name.split(" ")[0]} →`
            )}
          </button>

          <p className="social-disclaimer">
            🔒 By continuing, {isGoogle ? "Google" : "Apple"} will share your name and email address with Cartify to authorize your sign-in.
          </p>
        </div>
      </div>
    </div>
  );
}

export default SocialAuthModal;
